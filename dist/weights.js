...
exports.WeightMatrix = void 0;
class WeightMatrix {
    constructor(strings, maxFret) {
    ...
    pick() {
        ...
        const totalWeight = this.positions.reduce((sum, p) => sum + this.weights.get(`${p.stringName}:${p.fret}`), 0);
    ...
    update(stringName, fret, isCorrect) {
        const key = `${stringName}:${fret}`;
